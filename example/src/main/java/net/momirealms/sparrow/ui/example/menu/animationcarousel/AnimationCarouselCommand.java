...
public final class AnimationCarouselCommand {
    private static final String TARGET_ARGUMENT = "target";

    private AnimationCarouselCommand() {
    }

    @NotNull
    public static LiteralArgumentBuilder<CommandSourceStack> node() {
        return Commands.literal("animationcarousel")
                .then(Commands.argument(TARGET_ARGUMENT, ArgumentTypes.player())
                        .executes(AnimationCarouselCommand::open));
    }
    ...
    private static int open(@NotNull CommandContext<CommandSourceStack> context) throws CommandSyntaxException {
        ...
        AnimationCarouselMenu.open(target).whenComplete((ignoredResult, throwable) -> {
            if (throwable != null) {
