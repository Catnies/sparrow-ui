...
class SwitchingSignalTest {
    ...
    void switchingAwayReleasesThePreviousHandle() {
        MutableKeyedSignal<String, Integer> pages = KeyedSignal.of(String::length);
        AbstractKeyedSignal<String, Integer, ?> internal = (AbstractKeyedSignal<String, Integer, ?>) pages;
        MutableSignal<String> key = Signal.of("a");
        Signal<Integer> switching = Signals.switching(pages, key);
        ...
        assertEquals(1, internal.handleCount(), "只剩当前选中的那一个句柄");
    }
    ...
    void droppingTheSwitchingSignalReleasesItsHandle() {
        ...
        assertEquals(1, switching.get());
        assertEquals(1, internal.handleCount(), "选中的分区句柄由 switching 强持有");
        WeakReference<Signal<Integer>> probe = new WeakReference<>(switching);
        switching = null;
        host = null;
        GcSupport.awaitCollected(probe);

        assertEquals(0, internal.handleCount(), "switching 被回收后句柄不该赖在表里");
    }
    ...
    void keySwapDuringActivationClosesTheReplacedForwarding() {
        ...
        assertEquals(1, second.entryCount());
    }
    ...
    void deadHostReleasesTheChainEvenWhenEveryInvalidationIsSwallowed() {
        ...
        assertEquals(1, internalRaw.entryCount());
        dead = null;
        GcSupport.awaitCollected(probe);
        ...
        assertEquals(0, internalRaw.entryCount(), "持有方死亡后整条切换链应当解挂");
    }
    ...
    void failedVersionSnapshotLeavesTheSelectionUntouched() {
        ...
        assertEquals(1, ((AbstractSignal<?>) stable).entryCount(), "换源没成, 旧来源的转发要原样留着");
        assertEquals(1, ((AbstractSignal<?>) exploding).entryCount(), "失败的换源不该在新来源上留下转发");
        explode.set(false);

        assertEquals(20, switching.get());
        assertEquals(0, ((AbstractSignal<?>) stable).entryCount(), "换源成功才摘掉旧转发");
        assertEquals(2, ((AbstractSignal<?>) exploding).entryCount(), "测试自己那条, 加上换过来的这条");
    }
