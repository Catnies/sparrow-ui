...
import net.momirealms.sparrow.ui.state.internal.derive.MapDistinctSignal;
import net.momirealms.sparrow.ui.state.internal.derive.MappedSignal;
import net.momirealms.sparrow.ui.state.internal.time.DebounceSignal;
import net.momirealms.sparrow.ui.state.internal.time.ThrottleSignal;
...
public abstract class AbstractSignal<T> implements Signal<T> {
     ...
     */
    @NotNull
    protected final Subscription linkTo(@NotNull AbstractSignal<?> source, @NotNull Runnable listener) {
    ...
    @NotNull
    protected final Subscription[] linkAll(AbstractSignal<?>[] sources, @NotNull Runnable listener) {
    ...
    protected final void reapDeadEntries() {
    ...
    public static BiPredicate<Object, Object> defaultSameValue() {
        return DEFAULT_SAME_VALUE;
    }

    // 所有节点共用同一套空值判等. 两边都非 null 时才调用用户函数, ItemStack::isSimilar 一类方法引用可以直接传入.
    protected static <V> boolean same(BiPredicate<? super V, ? super V> sameValue, V current, V candidate) {
    ...
    public static <T> AbstractSignal<T> require(Signal<T> signal) {
        return (AbstractSignal<T>) Objects.requireNonNull(signal, "signal");
    }

    // 关闭一批可能尚未建立的订阅
    protected static void closeAll(Subscription @Nullable [] subscriptions) {
    ...
    // 值与版本的原子快照对.
    protected record Versioned<V>(V value, long version) {
        public Versioned {
        }
    }
