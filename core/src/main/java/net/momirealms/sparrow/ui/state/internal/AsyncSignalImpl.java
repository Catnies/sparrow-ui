...
@ApiStatus.Internal
public final class AsyncSignalImpl<T> extends AbstractSignal<T> implements AsyncSignal<T> {
    // 加载状态机
    ...
    public AsyncSignalImpl(T placeholder, Executor executor, Supplier<? extends T> loader, BiPredicate<? super T, ? super T> sameValue, @Nullable Polling polling) {
    ...
    @ApiStatus.Internal
    public record Polling(AbstractSignal<Long> clock, long periodNanos) {

        @NotNull
        public static Polling everyTicks(long periodTicks) {
            return new Polling(require(Signals.everyTicks(periodTicks)), TimeUnit.MILLISECONDS.toNanos(periodTicks * MILLIS_PER_TICK));
        }

        @NotNull
        public static Polling everyMillis(long periodMillis) {
